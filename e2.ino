// ============================================================
// ESL 296x128 BWR — Dooray 4 Rooms (v4.4 stable)
//  • Wi-Fi fetch → Wi-Fi OFF → BLE upload (간섭 최소화)
//  • Proto-A(0x01/0x02/0x03/0x05) 우선, 실패시 Proto-B(0x75 RAW → 0x76)
//  • CCCD: notify+indicate(0x0003) on CMD/IMG
//  • 상단 슬림 헤더(좌: 짧은 라벨, 우: 상태), 목록: “시간 예약자  제목”
//  • 다음 회의 경계(시작/종료) 직전에 자동 deep-sleep 기상
//  • MIRROR_X, 색 비트맵핑, 슬립 파라미터는 아래 설정
// ============================================================
...
// 표시 라벨은 짧게(“동/층” 제거) — 화면 상단 왼쪽에 씁니다.
struct Tag { const char* mac; const char* resId; const char* shortLabel; };
static Tag TAGS[] = {
...
GFXcanvas1 canvasBW (DRAW_W, DRAW_H);   // black/white
GFXcanvas1 canvasRED(DRAW_W, DRAW_H);   // red/white
U8G2_FOR_ADAFRUIT_GFX u8g2;

static const bool MIRROR_X         = true;   // 필요시 좌우 반전
static const bool BW_ONE_IS_WHITE  = true;   // canvas1: 1=white
static const bool RED_ONE_IS_WHITE = false;  // red layer: 1=white, 0=ink

// ------------------- Sleep policy -------------------
static const bool     SLEEP_ENABLE     = true;   // 개발중이면 false
static const uint32_t MIN_WAKE_SEC     = 300;    // 최소 5분
static const uint32_t MAX_WAKE_SEC     = 3600;   // 최대 60분
static const uint32_t BOUNDARY_MARGIN  = 60;     // 경계 60초 전에 기상
...
static bool busyNow(const RoomData& rd){
  if (!timeReady()) return false;
  time_t now=time(nullptr);
  for(int i=0;i<rd.count;i++) if (now>=rd.list[i].st && now<rd.list[i].ed) return true;
  return false;
}
static time_t nextBoundary(const RoomData& rd, time_t now){
  time_t best = 0;
  for(int i=0;i<rd.count;i++){
    if (rd.list[i].st>now && (best==0 || rd.list[i].st<best)) best = rd.list[i].st;
    if (rd.list[i].ed>now && (best==0 || rd.list[i].ed<best)) best = rd.list[i].ed;
  }
  return best;
}

// ------------------- UTF-8 safe fitting -------------------
static void utf8Chop(String& s){
  if (s.isEmpty()) return;
  int i = s.length()-1;
  while (i>0 && ((s[i] & 0xC0) == 0x80)) i--;
  s.remove(i);
}
static String fitUTF8Width(const String& text, int maxW){
  String s=text;
  u8g2.setFont(u8g2_font_unifont_t_korean2);
  while (u8g2.getUTF8Width(s.c_str()) > maxW && s.length()>0) utf8Chop(s);
  return s;
}

// ------------------- Render -------------------
static void drawHeader(const Tag& tag, bool isBusy, bool timeOK){
  // 얇은 헤더: 좌=짧은 라벨(굵게), 우=상태
  const int H = 22;
  ...
  String left = fitUTF8Width(String(tag.shortLabel), 168);
  u8g2.drawUTF8(8, 16, left.c_str());
  u8g2.drawUTF8(8, 15, left.c_str()); // bold 효과

  // 우측 상태
  if (!timeOK){
    const char* st="시간 동기화 중";
    int w=u8g2.getUTF8Width(st); int x=DRAW_W-8-w;
    u8g2.drawUTF8(x,16,st); u8g2.drawUTF8(x,15,st);
  } else if (isBusy){
    // 빨간 리본 (흰 글자)
    int W=86, Hh=18, rx=DRAW_W-8-W, ry=4;
    for(int yy=ry; yy<ry+Hh; ++yy)
      for(int xx=rx; xx<rx+W; ++xx) canvasRED.drawPixel(xx,yy,0);
    u8g2.begin(canvasRED); u8g2.setForegroundColor(1);
    const char* st="회의 중";
    int sx=rx+(W-u8g2.getUTF8Width(st))/2;
    u8g2.drawUTF8(sx, ry+14, st);
    u8g2.drawUTF8(sx, ry+13, st);
  } else {
    const char* st="비어 있음";
    int w=u8g2.getUTF8Width(st); int x=DRAW_W-8-w;
    u8g2.drawUTF8(x,16,st); u8g2.drawUTF8(x,15,st);
  ...
  canvasBW.drawFastHLine(0, H, DRAW_W, 0);
}
...
static void drawScreen(const Tag& tag, const RoomData& rd){
  ...
  const bool tOK = timeReady();
  drawHeader(tag, busyNow(rd), tOK);

  // 본문 목록 (최대 7줄, 간격 16px)
  const int y0=32, lh=16, maxW = DRAW_W-12-6;
  int shown = std::min(rd.count, 7);
  time_t now=time(nullptr);
  ...
  if (rd.count==0){
    ...
    u8g2.setFont(u8g2_font_unifont_t_korean2);
    u8g2.setForegroundColor(0); u8g2.setBackgroundColor(1);
    const char* msg = tOK? "오늘 예약 없음" : "네트워크/시간 대기";
    int w=u8g2.getUTF8Width(msg);
    u8g2.drawUTF8((DRAW_W-w)/2, 80, msg);
  } else {
    for(int i=0;i<shown;i++){
      int y=y0+i*lh;
      bool cur = tOK && now>=rd.list[i].st && now<rd.list[i].ed;

      // 진행중 빨간 점
      if (cur){ int cx=6, cy=y-6; for(int dy=-2;dy<=2;dy++) for(int dx=-2;dx<=2;dx++) if(dx*dx+dy*dy<=4) canvasRED.drawPixel(cx+dx,cy+dy,0); }

      // “HH:MM-HH:MM 예약자  제목”
      String head = hhmm(rd.list[i].startISO) + "-" + hhmm(rd.list[i].endISO) + " " + rd.list[i].who + "  ";
      String line = head + rd.list[i].subj;

      u8g2.begin(canvasBW); u8g2.setFont(u8g2_font_unifont_t_korean2);
      u8g2.setForegroundColor(0); u8g2.setBackgroundColor(1);
      line = fitUTF8Width(line, maxW);
      u8g2.drawUTF8(10, y, line.c_str());
    }
  }
...
static bool enableCCCD(BLERemoteCharacteristic* c){
  ...
  uint8_t v[2]={0x03,0x00}; // notify+indicate
...
static bool connectAndDiscover(const Tag& t){
  ...
  enableCCCD(g_cmd); enableCCCD(g_img);
  delay(120);
  return true;
}
...
static bool tryProtocolA(){
  ...
  uint8_t c3=0x03; cmdWrite(&c3,1,false); Serial.println("[A] TX 0x03");

  // 전송 시작
  uint32_t part=0, lastAck=0xFFFFFFFF;
  size_t dataSz = (size_t)std::min((size_t)g_partDataSize, (size_t)240);
  ...
  while (true){
    ...
    // ACK 없을 때 주기적 재전송
    if (millis()-g_lastTxMs > 1200 && !g_lastChunk.empty()){
      imgWrite(g_lastChunk.data(), g_lastChunk.size(), false);
      g_lastTxMs=millis();
    }
...
// ---------- Proto-B (RAW: 0x75 start → payload → 0x76 commit) ----------
...
static bool tryProtocolB(){
  ...
  while (off < total){
    ...
    off += n;
    if ((off/chunk)%10==0){ Serial.printf("[B] %u/%u\n",(unsigned)off,(unsigned)total); }
    delay(3);
  }
  ...
  delay(300); // 일부 기기 notify 없음 → 짧게 대기 후 종료
  return true;
}
...
static void maybeDeepSleepAll(){
  if (!SLEEP_ENABLE || !timeReady()) return;

  time_t now = time(nullptr);
  time_t best = 0;
  for(int i=0;i<NUM_TAGS;i++){
    time_t b = nextBoundary(g_room[i], now);
    if (b>0 && (best==0 || b<best)) best = b;
  }
  uint32_t sleepSec = MIN_WAKE_SEC; // 기본
  if (best>0){
    if (best > now + (time_t)BOUNDARY_MARGIN) {
      sleepSec = (uint32_t)((best - now) - BOUNDARY_MARGIN);
    }
  }
  if (sleepSec > MAX_WAKE_SEC) sleepSec = MAX_WAKE_SEC;
  if (sleepSec < MIN_WAKE_SEC) sleepSec = MIN_WAKE_SEC;

  Serial.printf("\n[Sleep] next wake in %u sec (min=%u, max=%u)\n",
                (unsigned)sleepSec, (unsigned)MIN_WAKE_SEC, (unsigned)MAX_WAKE_SEC);
  Serial.flush();
...
void setup(){
  ...
  Serial.println("\n====================================");
  Serial.println(" ESL 296x128 BWR — Dooray 4 Rooms (v4.4 stable)");
  Serial.println("====================================");
  ...
  for(int i=0;i<NUM_TAGS;i++){
    ...
    for(int attempt=0; attempt<3 && !ok; ++attempt){
      ...
      if (!ok){ Serial.printf("[Retry] %s (%d/3)\n", TAGS[i].shortLabel, attempt+1); delay(600); }
    }
    Serial.printf("[Result] %s: %s\n", TAGS[i].shortLabel, ok?"성공":"실패");
    if(i<NUM_TAGS-1) delay(800);
  }

  // 5) 다음 경계 직전 기상 (혹은 최소/최대 주기)
  maybeDeepSleepAll();
