// ============================================================
// ESL 296x128 BWR — Dooray 4 Rooms (최종 완성 버전)
// - 30분 단위 업데이트 (정시, 30분)
// - 진행 중 회의는 시간만 빨간색 표시
...
struct Tag { 
  ...
  uint32_t lastUpdate;
  bool needsUpdate;
  bool lastBusyState;  // 마지막 회의 상태 저장
};
...
static void drawScreen(const Tag& tag, const RoomData& rd){
  ...
  if (timeReady()) {
    ...
    if (tag.lastUpdate > 0) {
      struct tm lastLt;
      time_t lastTime = (time_t)tag.lastUpdate;
      localtime_r(&lastTime, &lastLt);
      const char* marker = (lastLt.tm_min < 30) ? "" : "·";
      snprintf(dateStr, sizeof(dateStr), "%d.%d(%s) %02d%s", 
               lt.tm_mon+1, lt.tm_mday, weekdays[lt.tm_wday], 
               lastLt.tm_hour, marker);
    } else {
  ...
  if(rd.count == 0){
    ...
    for(int i=0; i<shown; i++){
      ...
      // 제목 처리
      char subj[80];
      strncpy(subj, rd.list[i].subj, 79);
      subj[79] = '\0';
      if(strlen(subj) > 42) {
        subj[40] = '.';
        subj[41] = '.';
      ...
      if (isNow) {
        ...
        u8g2.setForegroundColor(0);
        char restLine[128];
        snprintf(restLine, sizeof(restLine), " %s/%s", who, subj);
        int timeWidth = u8g2.getUTF8Width(timeStr);
        u8g2.drawUTF8(listX+2+timeWidth, y, restLine);
      } else {
        // 전체 검정색
        u8g2.begin(*canvasBW);
        u8g2.setForegroundColor(0);
...
static void processUpdates(bool forceUpdate = false) {
  ...
  for(int i = 0; i < NUM_TAGS; i++) {
    ...
    if (forceUpdate || dataChanged || statusChanged || timeExpired) {
      TAGS[i].needsUpdate = true;
      TAGS[i].lastBusyState = isBusy;
      updateCount++;
      Serial.printf("UPDATE (data:%d, status:%d)\n", dataChanged, statusChanged);
    } else {
...
void setup(){
  ...
  Serial.println("\n╔════════════════════════════════════╗");
  Serial.println("║   ESL Dooray Final v4.0           ║");
  Serial.println("║   30분 단위 스마트 업데이트        ║");
  Serial.println("╚════════════════════════════════════╝");
...
void loop() {
  ...
  if (timeReady()) {
    ...
    if (lt.tm_min == 0 || lt.tm_min == 30) {
      ...
      if (lt.tm_hour != lastHour || lt.tm_min != lastMin) {
        lastHour = lt.tm_hour;
        lastMin = lt.tm_min;
        Serial.printf("\n[TIME] %02d:%02d - Starting update\n", lt.tm_hour, lt.tm_min);
